...
class VoiceDigestNotifierTests(unittest.TestCase):
    ...
    def test_build_setup_report_marks_destination_gap_as_blocked(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(report["payload_details"]["path"], str(payload_path))
            self.assertEqual(report["handoff_text_details"]["path"], str(handoff_path))
            self.assertIn("modified_at", report["payload_details"])
            self.assertIn("age_minutes", report["handoff_text_details"])
    ...
    def test_main_check_setup_json_reports_ready_environment(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(result["payload_details"]["path"], str(payload_path))
            self.assertEqual(result["handoff_text_details"]["path"], str(handoff_path))
            self.assertIn("modified_at", result["payload_details"])
            self.assertIn("age_minutes", result["handoff_text_details"])
            self.assertEqual(result["diagnostics"]["config_exists"], True)
