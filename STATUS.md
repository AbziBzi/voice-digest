...
## Immediate next step
Point the scheduler at Edwin's intended upstream digest input path (or populate `incoming_digests/`) and provision Edwin's real Signal/OpenClaw destination via scheduler env vars or a local `.voice_digest_notifier.json`; use `python3 scripts/voice_digest_openclaw_notifier.py --check-setup --json` to confirm the notifier environment is actually ready, then repeat the now-proven `scripts/voice_digest_dispatch_job.py --send --openclaw-dry-run` check in that intended live environment before one true live dispatch run against Edwin's real target.
